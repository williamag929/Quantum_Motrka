...
A local-first AI assistant ecosystem: a small model on your own machine (Gemma 4 via Ollama) answers what it can, Claude handles the rest, and (in the CLI today) private data never reaches the cloud in clear. Entry points span a CLI, VS Code extension, browser extension, and a system tray daemon.

**Measured, not claimed.** On 29 held-out real-world requests, on a CPU-only desktop, the CLI's router kept **45%** of requests on the machine, sent **0 of 5** private requests to the cloud in clear (Claude-only: 5 of 5), and averaged quality **8.0 / 10** vs 8.8 for Claude-only. See [`dual_ai/bench/reports/`](dual_ai/bench/reports/) and [`dual_ai/bench/README.md`](dual_ai/bench/README.md).
